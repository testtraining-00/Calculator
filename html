...
<body class="min-h-screen bg-gray-50 flex items-center justify-center p-4 sm:p-8 font-sans text-gray-800">
  ...
  <div class="max-w-5xl w-full bg-white rounded-2xl shadow-xl overflow-hidden border-t-8" style="border-color: #ff5400;">
    ...
    <div class="p-6 sm:p-10 bg-white border-b border-gray-100">
      <div class="flex items-center gap-4 mb-4">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Multi-Month Upfront Offer Calculator</h1>
      </div>
      <p class="text-gray-600 text-lg leading-relaxed max-w-3xl">
        The most effective strategy when pitching multi-month offers is to compare the combined cost of the discounted device and the multi-month plan fee against the full Suggested Retail Price (SRP).
      </p>
    </div>
      ...
      <div class="lg:col-span-5 p-6 sm:p-10 bg-gray-50 border-r border-gray-100">
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <input
              type="range" min="100" max="1500" step="10" value="600" id="srp-input"
              class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
            />
          </div>

          <!-- Slider 2: Discounted Price -->
          <div class="space-y-2">
            <div class="flex justify-between items-center">
          ...
          <!-- Slider 3: Monthly Fee -->
          <div class="space-y-2">
            <div class="flex justify-between items-center">
              <label class="text-sm font-semibold text-gray-700">Monthly Plan Fee</label>
              <span class="font-bold" style="color: #ff5400;" id="fee-val">$50.00</span>
            </div>
            <input
              type="range" min="10" max="150" step="5" value="50" id="monthly-fee-input"
              class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
            />
          </div>
          ...
          <div class="space-y-2">
            <div class="flex justify-between items-center">
              ...
              <span class="font-bold" style="color: #ff5400;" id="months-val">6 Months</span>
            </div>
            <input
              type="range" min="2" max="12" step="1" value="6" id="months-input"
              class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
            />
          </div>
        </div>
        ...
        <div class="mt-8 p-4 rounded-xl border" style="background-color: #fff0e6; border-color: #ffd9c2;">
          ...
          <p class="text-sm text-gray-800 leading-tight">
            "Instead of paying <span class="pitch-srp"></span> just for the phone, you can get the phone <strong>AND <span id="pitch-months"></span> months of service</strong> for <span id="pitch-compare"></span> the same price (<strong id="pitch-total"></strong>)!"
          </p>
        </div>
      </div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white border-2 rounded-2xl p-5 relative overflow-hidden shadow-lg transform transition-all hover:scale-[1.01]" style="border-color: #ff5400;">
            ...
            <div class="flex justify-between items-start mb-4 relative z-10">
              <div>
                <h3 class="text-lg font-black uppercase tracking-wide" style="color: #ff5400;">Multi-Month Offer</h3>
            ...
            <div class="space-y-3 relative z-10">
              <div class="flex justify-between items-center text-sm">
                <span class="flex items-center gap-2 font-bold text-gray-800">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ff5400" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"></rect><line x1="12" y1="18" x2="12.01" y2="18"></line></svg>
                  Discounted Device Price
                </span>
                <span class="font-bold text-lg" style="color: #ff5400;" id="mm-discount"></span>
              </div>
              <div class="flex justify-between items-center text-sm pb-3 border-b border-gray-100">
                <span class="flex items-center gap-2 font-bold text-gray-800">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ff5400" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                  <span id="mm-months-text"></span>
                </span>
                <span class="font-bold text-lg" style="color: #ff5400;" id="mm-plan-total"></span>
              </div>
  ...
  <script>
    // State logic transferred to standard JS
    ...
    function calculate() {
      ...
      // Math
      const standardTotal = srp + monthlyFee;
      const multiMonthTotal = discount + (monthlyFee * months);
      const multiMonthPlanTotal = monthlyFee * months;
      const diffVsSRP = multiMonthTotal - srp;
      ...
      document.getElementById('pitch-months').innerText = months;
      document.getElementById('pitch-compare').innerText = diffVsSRP <= 0 ? 'less than' : 'around';
      document.getElementById('pitch-total').innerText = formatCurrency(multiMonthTotal);
      ...
      document.querySelectorAll('.std-fee').forEach(el => el.innerText = formatCurrency(monthlyFee));

      // Multi-Month Option Details
      document.getElementById('mm-total').innerText = formatCurrency(multiMonthTotal);
      document.getElementById('mm-discount').innerText = formatCurrency(discount);
      document.getElementById('mm-months-text').innerText = `${months}-Month Upfront Plan Total`;
      document.getElementById('mm-plan-total').innerText = formatCurrency(multiMonthPlanTotal);
